...
const TaskItem = memo(function TaskItem({ task, onToggle, onUpdate, onDelete }) {
  ...
  function renderTaskTitle() {
    const titleClass = `block break-words text-sm leading-6 transition ${task.completed ? 'text-slate-500 line-through' : 'text-slate-200 hover:text-cyan-300'}`

    if (task.isOptimistic) {
      return <p className={titleClass}>{task.title}</p>
    }
  ...
  if (isEditing) {
    return <TaskEditor task={task} onSave={handleSave} onCancel={handleCancel} isSaving={isSaving} />
  }
    ...
    <div className='rounded-xl border border-slate-800 bg-slate-950 p-4'>
      <div className='flex flex-col gap-3 sm:flex-row sm:items-start'>
        <div className='flex min-w-0 flex-1 items-start gap-3'>
          <button type='button' onClick={() => onToggle(task._id)} disabled={task.isOptimistic} className='mt-0.5 shrink-0 text-slate-500 transition hover:text-cyan-400 disabled:cursor-not-allowed disabled:opacity-50'>
        ...
        <div className='flex shrink-0 items-center justify-end gap-1 sm:pt-0.5'>
          <button type='button' onClick={() => setIsEditing(true)} disabled={task.isOptimistic || isSaving} className='rounded-lg p-2 text-slate-500 transition hover:bg-cyan-500/10 hover:text-cyan-400 disabled:cursor-not-allowed disabled:opacity-40'>
            <Pencil size={17} />
          </button>

          <button type='button' onClick={handleDelete} disabled={isDeleting || task.isOptimistic} className='rounded-lg p-2 text-slate-500 transition hover:bg-red-500/10 hover:text-red-400 disabled:cursor-not-allowed disabled:opacity-50'>
